...
func (wq *WorkoutQuery) FirstIDX(ctx context.Context) int {
...
func (wq *WorkoutQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (wq *WorkoutQuery) loadExercises(ctx context.Context, query *ExerciseQuery, nodes []*Workout, init func(*Workout), assign func(*Workout, *Exercise)) error {
	...
	query.Where(predicate.Exercise(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workout.ExercisesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.workout_exercises
		if fk == nil {
			return fmt.Errorf(`foreign-key "workout_exercises" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workout_exercises" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (wq *WorkoutQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(workout.Table, workout.Columns, sqlgraph.NewFieldSpec(workout.FieldID, field.TypeInt))
	_spec.From = wq.sql
