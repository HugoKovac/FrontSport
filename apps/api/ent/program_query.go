...
func (pq *ProgramQuery) QueryUser() *UserQuery {
...
func (pq *ProgramQuery) QueryExercises() *ExerciseQuery {
	query := (&ExerciseClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProgramQuery) FirstIDX(ctx context.Context) int {
...
func (pq *ProgramQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pq *ProgramQuery) Clone() *ProgramQuery {
	...
	return &ProgramQuery{
		...
		withExercises: pq.withExercises.Clone(),
...
func (pq *ProgramQuery) WithUser(opts ...func(*UserQuery)) *ProgramQuery {
	query := (&UserClient{config: pq.config}).Query()
...
func (pq *ProgramQuery) WithExercises(opts ...func(*ExerciseQuery)) *ProgramQuery {
	query := (&ExerciseClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withExercises = query
	return pq
}
...
func (pq *ProgramQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Program, error) {
	...
	if query := pq.withExercises; query != nil {
		if err := pq.loadExercises(ctx, query, nodes,
			func(n *Program) { n.Edges.Exercises = []*Exercise{} },
			func(n *Program, e *Exercise) { n.Edges.Exercises = append(n.Edges.Exercises, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProgramQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Program, init func(*Program), assign func(*Program, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Program)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (pq *ProgramQuery) loadExercises(ctx context.Context, query *ExerciseQuery, nodes []*Program, init func(*Program), assign func(*Program, *Exercise)) error {
	...
	query.Where(predicate.Exercise(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(program.ExercisesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.program_exercises
		if fk == nil {
			return fmt.Errorf(`foreign-key "program_exercises" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "program_exercises" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProgramQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withUser != nil {
			_spec.Node.AddColumnOnce(program.FieldUserID)
		}
	}
