...
func CheckWorkoutExerciseExists(workoutExerciseService ports.WorkoutExerciseService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		var query dto.WorkoutExerciseQuery
		v := validator.New()
		...
		wrkEx, err := workoutExerciseService.GetWorkoutExerciseByIdFromUser(query.WorkoutExerciseID, user.Id)
		if ent.IsNotFound(err) {
			c.Set("HX-Redirect", "/error/not-found")
			return nil
		}
		fibercontext.SetWorkoutExerciseToContext(c, wrkEx)
		return c.Next()
	}
}
